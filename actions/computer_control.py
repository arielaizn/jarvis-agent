...
def _type(text: str, interval: float = 0.03) -> str:
    ...
    if not text.isascii():
        if not _PYPERCLIP:
            raise RuntimeError("נדרשת ספריית pyperclip להקלדה בעברית.")
        pyperclip.copy(text)
        pyautogui.hotkey("command" if _get_os() == "mac" else "ctrl", "v")
    else:
        pyautogui.typewrite(text, interval=interval)
    return f"הוקלד: {text[:60]}{'…' if len(text) > 60 else ''}"


def _smart_type(text: str, clear_first: bool = True) -> str:
    _require_pyautogui()
    if not text.isascii() and not _PYPERCLIP:
        raise RuntimeError("נדרשת ספריית pyperclip להקלדה בעברית.")
    if clear_first:
        _clear_field()
        time.sleep(0.1)

    if (len(text) > 20 or not text.isascii()) and _PYPERCLIP:
        pyperclip.copy(text)
        time.sleep(0.1)
        paste_key = "command" if _get_os() == "mac" else "ctrl"
        pyautogui.hotkey(paste_key, "v")
        return f"הטקסט הודבק: {text[:60]}{'…' if len(text) > 60 else ''}"
...
def _focus_window(title: str) -> str:
    if not isinstance(title, str) or not title.strip() or len(title) > 300:
        return "יש להזין שם חלון באורך 1 עד 300 תווים."
    os_name = _get_os()
    ...
    if os_name == "mac":
        ...
        try:
            ...
            if result.returncode != 0:
                return f"הבאת החלון לקדמה נכשלה: {result.stderr.strip()}"
            time.sleep(0.3)
            return f"החלון הובא לקדמה: {title}"
        except Exception as e:
            return f"הבאת החלון לקדמה נכשלה: {e}"
...
def computer_control(
    ...
    # Typed text and clipboard contents may contain credentials.
