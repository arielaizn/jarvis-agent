...
ASR_MODEL           = "gemini-3.8-live"
LIVE_MODEL          = ASR_MODEL  # explicit speech-only transport, never a brain fallback
...
class JarvisLive:
    def __init__(self, ui: JarvisUI):
        self.ui             = ui
        self._asst_name     = "ג׳רוויס"   # updated each session from config
        self.session              = None
        ...
        self.ui.on_api_key_change = self._on_api_key_change
        self.ui.on_brain_change = self._on_brain_change
        self._native_codex = NativeCodexClient()
        self._pending_brain_selection = None
        self._brain_settings_lock = asyncio.Lock()
        self._mode_epoch = 0
        self._session_mode_epoch = 0
        self._session_galaxy_mode = False
        self._galaxy_tts_tail_until = 0.0
        self._hud_brain = HudBrain()
        self._hud_generation = 0
        self.ui.on_galaxy_mode_change = self._on_galaxy_mode_change
    ...
    def _in_galaxy_mode(self) -> bool:
        # A strict bool avoids accidentally treating an absent/mock UI property
        # as an enabled mode. Only the native view switch owns this flag.
        return getattr(getattr(self, "ui", None), "galaxy_mode", False) is True

    def _transport_current(self) -> bool:
        return (getattr(self, "_session_mode_epoch", 0)
                == getattr(self, "_mode_epoch", 0))

    def _galaxy_isolated(self) -> bool:
        """Block the legacy assistant throughout either direction of a swap."""
        return (self._in_galaxy_mode()
                or getattr(self, "_session_galaxy_mode", False) is True
                or not self._transport_current())

    def _galaxy_mic_blocked(self) -> bool:
        """Keep native speech, including ephemeral drift labels, out of ASR."""
        if not self._transport_current():
            return True
        # The focus panel can speak while hidden behind the HUD. This guard
        # must run before any workspace-specific microphone logic.
        if getattr(self.ui, "galaxy_speaking", False) is True:
            self._galaxy_tts_tail_until = time.monotonic() + GALAXY_TTS_TAIL_SECONDS
            return True
        return bool(self.ui.muted or time.monotonic()
                    < getattr(self, "_galaxy_tts_tail_until", 0.0))

    def _on_galaxy_mode_change(self, enabled: bool) -> None:
        """Replace the transport context when the native workspace changes."""
        self._mode_epoch += 1
        self._hud_generation += 1
        self._hud_brain.clear()
        ...
        self.request_reconnect(keep_context=False, reason="סביבת עבודה")
    ...
    def _on_wake_detected(self) -> None:
        ...
        self.wake(reason="זוהתה מילת ההפעלה")
    ...
    def wake(self, reason: str = "wake word") -> None:
        ...
        self.ui.write_log(f"SYS: ג׳רוויס פעיל. {reason}")
    ...
    def sleep(self, reason: str = "timeout") -> None:
        ...
        self.ui.write_log(f"SYS: ממתין להפעלה. {reason}. אפשר לומר Hey Jarvis.")

    async def _run_sleep_watch(self) -> None:
        """Auto-sleep after the configured silence window (wake-word mode only)."""
        while True:
            await asyncio.sleep(5)
            if self._galaxy_isolated() or not self._wake_enabled or not self._awake:
                continue
            ...
            if (time.monotonic() - self._last_user_speech) > self._wake_sleep_timeout:
                self.sleep(reason="לא זוהה דיבור במשך שתי דקות")
    ...
    def _ui_wake_toggle(self, enable: bool) -> str:
        ...
        if enable:
            ...
            self.sleep(reason="מילת ההפעלה הופעלה")
            return "enabled"
        else:
            self._wake_enabled = False
            save_wake_word_enabled(False)
            self.wake(reason="מילת ההפעלה כובתה")
            return "disabled"
    ...
    def _ui_wake_manual(self) -> None:
        ...
        if not self._wake_enabled:
            return
        if self._awake:
            self.sleep(reason="לחיצה על המתנה")
        else:
            self.wake(reason="לחיצה על הפעלה")
    ...
    def plugin_say(self, instruction: str) -> None:
        """Speak a plugin progress line locally, outside both model histories."""
        if not self._in_galaxy_mode():
            self.ui.local_say(instruction)
    ...
    def _on_voice_change(self):
        """Local speech reads its saved profile on the next utterance."""
        self.ui.local_say("הקול עודכן, אדוני. אני מוכן למשימה הבאה.")

    def _on_api_key_change(self):
        """Native settings saved a private key; rebuild ASR with that credential."""
        from core import gemini
        gemini.api_key(refresh=True)
        # Discard any old in-flight HUD answer. Subsequent turns construct a
        # fresh client. The separate galaxy server rereads its private file.
        self._hud_generation += 1
        self._resume_handle = None
        self._conn_backoff = 0
        self.request_reconnect(keep_context=False, reason="מפתח Gemini עודכן")

    def _on_brain_change(self, provider=None):
        """The settings buttons select the shared server's persisted provider."""
        if provider not in {None, "codex", "gemini"}:
            self.ui.update_brain_state({"error": "בחר Codex או Gemini."})
            return
        loop = getattr(self, "_loop", None)
        if loop is None:
            self._pending_brain_selection = provider
            return
        asyncio.run_coroutine_threadsafe(self._apply_brain_selection(provider), loop)

    async def _apply_brain_selection(self, provider=None):
        # Keep a slower initial status read from overwriting a newer selection.
        if not hasattr(self, "_brain_settings_lock"):
            self._brain_settings_lock = asyncio.Lock()
        async with self._brain_settings_lock:
            await self._apply_brain_selection_locked(provider)

    async def _apply_brain_selection_locked(self, provider=None):
        try:
            result = await self._native_codex.select(provider)
            if provider is not None:
                self._hud_generation += 1
                self._hud_brain.clear()
                self._session_log = []
            self.ui.update_brain_state(result)
            if provider is not None:
                message = result.get("answer", "המוח עודכן.")
                self.ui.write_log(f"SYS: {message}")
        except NativeTaskError as error:
            self.ui.update_brain_state({"error": str(error)})

    async def _send_codex_command(self, text, cancelled):
        """A completed voice transcript and typed text take the same task path."""
        self.ui.set_state("THINKING")
        try:
            result = await self._native_codex.ask(
                text, cancelled=cancelled,
                progress=lambda message: self.ui.write_log(f"SYS: {message}"))
            if cancelled() or result is None:
                return
            answer = result["answer"]
            self.ui.write_log(f"{self._asst_name}: {answer}")
            self.ui.show_content(result.get('model_label', "Codex · GPT 6 Astra · High"), answer)
            if result.get('provider') == 'gemini':
                self.ui.update_brain_state(await self._native_codex.state())
            # Keep spoken task reports brief. Full details remain on screen.
            spoken = re.split(r"(?<=[.!?])\s+|\n+", answer.strip())
            self.ui.local_say(" ".join(spoken[:3])[:600])
        except NativeTaskError as error:
            if not cancelled():
                self.ui.write_log(f"ERR: {error}")
                self.ui.local_say(str(error))
        finally:
            if not cancelled():
                self.ui.set_state("THINKING" if self._native_codex.busy else ("SLEEPING" if self.ui.muted else "LISTENING"))
    ...
    def _on_audio_device_change(self):
        ...
        self.request_reconnect(keep_context=True, reason="התקן שמע")
    ...
    async def _watch_reconnect(self):
        ...
        self.ui.write_log(
            f"SYS: מעדכן {reason} ומתחבר מחדש"
            + ("..." if keep else " ופותח שיחה חדשה…")
        )
    ...
    def _make_remote_key(self):
        ...
        if self._dashboard is None:
            self.ui.write_log(
                "SYS: לוח הבקרה אינו זמין. "
                "להתקנה: pip install fastapi \"uvicorn[standard]\" cryptography"
            )
            return None
    ...
    async def _gemini_background_allowed(self):
        """Legacy proactive generation belongs only to the selected Gemini mode."""
        try:
            state = await self._native_codex.state()
            return state.get("provider") == "gemini"
        except NativeTaskError:
            return False

    async def _send_user_command(self, text: str, *, background=False):
        if self._in_galaxy_mode():
            if not background:
                self.ui.galaxy_utterance(text)
            return
        if getattr(self, "_native_codex", None) is not None and re.fullmatch(
                r"\s*(?:stop|cancel(?: (?:the )?task)?|עצור|תעצור|בטל(?: את המשימה)?|תבטל(?: את המשימה)?)\s*[.!?]?", text, re.I):
            self.interrupt()
            self.ui.local_say("נשלחה בקשת עצירה למשימה.")
            return
        epoch = self._mode_epoch
        generation = self._hud_generation
        cancelled = lambda: (self._in_galaxy_mode() or self._mode_epoch != epoch
                             or self._hud_generation != generation)
        if not background:
            self.ui.acknowledge()
        if not background and re.search(
                r"(?:מה\b.*(?:במסך|על המסך)|תסתכל\b.*מסך|what\b.*(?:on (?:my |the )?screen)|look at (?:my |the )?screen)", text, re.I):
            await self._send_live_screen(text, cancelled)
            return
        try:
            provider_state = await self._native_codex.state()
            if cancelled():
                return
            self.ui.update_brain_state(provider_state)
            if provider_state.get("provider") == "codex" and background:
                return
            if provider_state.get("provider") in {"codex", "gemini"} and not background:
                if not background:
                    await self._send_codex_command(text, cancelled)
                return
            if provider_state.get("provider") != "gemini":
                raise NativeTaskError("בחר Codex או Gemini דרך ההגדרות כדי לשלוח פקודה.")
        except NativeTaskError as error:
            if not cancelled():
                self.ui.write_log(f"ERR: {error}")
                self.ui.local_say(str(error))
            return
        try:
            context = await asyncio.to_thread(skill_prompt_context, text)
            if cancelled():
                return
            parts = ([types.Part(text="Requested local skill instructions (use within the user task):\n" + context)] if context else [])
            parts.append(types.Part(text=text))
            self.ui.set_state("THINKING")
            config = await asyncio.to_thread(self._build_hud_config)
            result = await self._hud_brain.ask(
                parts, config,
                lambda function: self._execute_tool(function, trusted_hud=True),
                cancelled=cancelled, take_vision=self._take_hud_vision)
            if cancelled():
                return
            answer = result["answer"]
            self.ui.write_log(f"{self._asst_name}: {answer}")
            self.ui.show_content("Gemini 3.8 Flash", answer)
            self.ui.local_say(answer)
            self._session_log.extend([f"User: {text}", f"{self._asst_name}: {answer}"])
            self._session_log = self._session_log[-40:]
        except TurnSuperseded:
            pass
        except Exception as error:
            if not cancelled():
                status = error.status if isinstance(error, BrainUnavailable) else None
                message = ("המכסה של Gemini 3.8 Flash מוצתה כרגע. הבקשה לא הושלמה."
                           if status == 429 else
                           "Gemini 3.8 Flash אינו זמין כרגע. הבקשה לא הושלמה.")
                self.ui.write_log(f"ERR: {message}")
                self.ui.local_say(message)
                print(f"[HUD brain] {BRAIN_MODEL}: {type(error).__name__}; status={status}")
        finally:
            if not cancelled():
                self.ui.set_state("SLEEPING" if self.ui.muted else "LISTENING")
                if self._vision_cam_active:
                    self.ui.stop_camera_stream()
                    self._vision_cam_active = False
                self._vision_busy = False

    async def _send_live_screen(self, text, cancelled):
        from core.gemini_live import generate, wav_data, LiveError
        self.ui.set_state('THINKING')
        try:
            image, media_type = await asyncio.to_thread(_capture_screen)
            if cancelled():
                return
            if media_type != 'image/jpeg':
                raise LiveError('לכידת המסך לא החזירה תמונת JPEG תקינה.')
            result = await generate(text, jpeg=image, source='screen')
            if cancelled():
                return
            self.ui.show_content('Gemini 3.8 Live · מסך', result['answer'])
            self.ui.write_log(f"{self._asst_name}: {result['answer']}")
            self.ui.play_live_audio(wav_data(result['pcm']))
        except LiveError as error:
            if not cancelled():
                self.ui.show_content('Gemini 3.8 Live', str(error))
        except Exception:
            if not cancelled():
                self.ui.show_content('Gemini 3.8 Live', 'לא הצלחתי ללכוד או לבדוק את המסך. נסה שוב.')
        finally:
            if not cancelled():
                self.ui.set_state('SLEEPING' if self.ui.muted else 'LISTENING')
    ...
    def set_push_to_talk(self, enabled: bool) -> str:
        ...
        try:
            self.ui.write_log(
                f"SYS: דיבור בלחיצה פעיל. החזק את {self._ptt.label}"
                + ("." if scope == "global"
                   else " (פעיל כשהחלון נמצא בקדמת המסך)."))
        except Exception:
            pass
        return scope
    ...
    def interrupt(self) -> None:
        ...
        from core.browser_runtime import cancel as cancel_browser
        self._hud_generation += 1
        client = getattr(self, "_native_codex", None)
        loop = getattr(self, "_loop", None)
        if client is not None and loop is not None:
            asyncio.run_coroutine_threadsafe(client.cancel(), loop)
        cancelled = cancel_browser()
        if cancelled.get("success"):
            self.ui.write_log("SYS: נשלחה בקשת עצירה למשימת הדפדפן.")
        self._interrupted = True
    ...
    def speak(self, text: str):
        if not self._in_galaxy_mode():
            self.ui.local_say(text)

    def speak_error(self, tool_name: str, error: str):
        short = str(error)[:120]
        self.ui.write_log(f"ERR: {tool_name}: {short}")
        self.speak(f"דווח בעברית על שגיאה בכלי {tool_name}: {short}")
    ...
    def _build_hud_config(self) -> types.GenerateContentConfig:
        ...
        try:
            ...
            self._asst_name = (_cfg.get("assistant_name") or "ג׳רוויס").strip()
            _user_name = (_cfg.get("user_name") or "").strip()
        except Exception:
            self._asst_name = "ג׳רוויס"
            _user_name = ""
        ...
        # The explicit address form overrides stored names and older prompts.
        _addr = "ADDRESS: Always address the user as אדוני, never by first name."
        identity_ctx = (
            f"[IDENTITY]\n"
        ...
        _all_decls = [{key: value for key, value in declaration.items() if key != "behavior"}
        ...
        parts = [time_ctx, identity_ctx, "[LANGUAGE] Speak and write Hebrew by default. Use natural Israeli Hebrew. Keep tool IDs, code, paths, and product names unchanged. Always address the user as אדוני, never by first name. This overrides older memory and prompts."]
        if mem_str:
            parts.append(mem_str)
        parts.append(sys_prompt)

        parts.append(
            "[BRAIN] Your reasoning model is gemini-3.8-flash. The separate Live "
            "connection only transcribes speech and never answers or runs tools. "
            "Use tools for real actions and report only their actual results. "
            "For a request about the current screen or webcam, capture a fresh "
            "frame with screen_process; earlier frames do not establish the current view. "
            "Keep spoken answers brief; detailed content can remain on screen."
        )
        return types.GenerateContentConfig(
            system_instruction="\n".join(parts),
            tools=[types.Tool(function_declarations=_all_decls)],
    ...
    def _tuning_config(self) -> dict:
        ...
        # Gemini 3.8 Live rejects thinking_config, including thinking_level.

        return out

    async def _execute_tool(self, fc, *, trusted_hud=False) -> types.FunctionResponse:
        if not trusted_hud or self._in_galaxy_mode():
            return types.FunctionResponse(
                id=fc.id, name=fc.name,
                response={"error": "Tools are disabled in the transcription transport."},
            )
        name = fc.name
        args = dict(fc.args or {})

        print(f"[JARVIS] tool: {name}")
        self.ui.set_state("THINKING")
        ...
        try:
            if name == "recall_memory":
                ...
                async def _do_shutdown():
                    ...
                    self.ui.local_say("להתראות")
                    await asyncio.sleep(1.5)
                    await asyncio.to_thread(_cleanup_integrations)
                    import os as _os
                    _os._exit(0)
    ...
    async def _listen_audio(self):
        ...
        def callback(indata, frames, time_info, status):
            galaxy_mode = self._in_galaxy_mode()
            if self._galaxy_mic_blocked():
                return
            # ── Wake-word gate ───────────────────────────────────────────────
            ...
            if not galaxy_mode and self._wake_enabled and not self._awake:
            ...
            if not galaxy_mode and self._ptt_enabled and not self._ptt_held:
                return
        ...
        try:
            ...
            while True:
                # In the native galaxy, ears-off closes the hardware stream.
                # Background organs cannot open it by changing their own state.
                if self.ui.muted:
                    await asyncio.sleep(0.1)
                    continue
                # Resolving an explicitly chosen device can validate its input
                # transport. Do that only after the physical ear button, never
                # as part of startup discovery or while ears are off.
                _mic_dev = audio_devices.resolve(_mic_name, "input", probe_input=True)
                if self.ui.muted:
                    continue
                ...
                try:
                    ...
                    self.ui.write_log(
                        f"SYS: המיקרופון {_mic_name} אינו זמין. משתמש בברירת המחדל."
                    )
    ...
    def _take_hud_vision(self):
        """Consume one newly captured frame for the REST brain only."""
        pending = self._pending_vision
        self._pending_vision = None
        if not pending:
            return []
        image, media_type, question, angle = pending
        source = "WEBCAM" if angle == "camera" else "SCREEN CAPTURE"
        self._vision_busy = False
        return [types.Part.from_bytes(data=image, mime_type=media_type),
    ...
    async def _play_audio(self):
        ...
        try:
            ...
            self.ui.write_log(f"SYS: הרמקול {_spk_name} אינו זמין. משתמש בברירת המחדל.")
    ...
    async def _send_startup_briefing(self) -> None:
        ...
        """
        if self._galaxy_isolated() or not await self._gemini_background_allowed():
            return
        memory   = load_memory()
        ...
        if not self.session or self._galaxy_isolated():
            return
        ...
        await self._send_user_command(p1, background=True)
        ...
        async def _deliver_news():
            try:
                ...
                try:
                    ...
                    self.ui.write_log(f"SYS: טעינת החדשות נכשלה: {e!r}")
                    news_text = ""
                ...
                if not failed:
                    ...
                    self.ui.write_log(
                        f"SYS: החדשות אינן זמינות. תשובת השרת: {news_text[:120]!r}"
                    )
                    p2 = (
                ...
                await self._send_user_command(p2, background=True)
    ...
    async def _save_session_summary(self) -> None:
        ...
        try:
            ...
            if summary and not self._galaxy_isolated():
    ...
    async def _run_system_monitor(self) -> None:
        ...
        while True:
            ...
            if self._galaxy_isolated() or not alert or not self.session or not self._awake:
                continue
            ...
            if speaking or (time.monotonic() - self._last_user_speech) < 10:
                continue
            try:
                await self._send_user_command(alert, background=True)
            except Exception as e:
    ...
    async def _run_background_monitor(self) -> None:
        ...
        while True:
            if (self.session and self._awake and not self._galaxy_isolated()
                    and await self._gemini_background_allowed()):
                ...
                if not speaking and not recent_speech:
                    try:
                        ...
                        for alert in alerts:
                            ...
                            await self._send_user_command(msg, background=True)
    ...
    async def _run_proactive_mode(self) -> None:
        ...
        while True:
            ...
            if (self._galaxy_isolated() or not self.session or not self._awake
                    or not await self._gemini_background_allowed()):
                continue
            ...
            try:
                ...
                await self._send_user_command(prompt, background=True)
        ...
        while True:
            try:
                ...
                if self.session:
                    ...
                    if self._wake_enabled and not self._awake:
                        self.wake(reason="פקודה מרחוק")
                    await self._send_user_command(text)
    ...
    async def run(self):
        ...
        # Brain/task requests do not depend on a working speech connection.
        asyncio.create_task(self._apply_brain_selection(self._pending_brain_selection))
        ...
        while True:
            try:
                ...
                self._session_galaxy_mode = self._in_galaxy_mode()
                ...
                async with (
                    ...
                    if self._session_galaxy_mode:
                        # The native ear button is the only microphone gate in
                        # this mode; wake-word settings belong to the HUD.
                        self._awake = True
                        self.ui.set_state("SLEEPING" if self.ui.muted else "LISTENING")
                    ...
                    if not self._session_galaxy_mode:
                        tg.create_task(self._run_sleep_watch())
                        if self._dashboard:
                            tg.create_task(self._relay_phone_audio())

                    # ASR never starts a proactive brain or greeting.
                ...
                if self._enhanced_live and (
                    ...
                    self.ui.write_log(
                        "SYS: זיהוי הפנייה בקול אינו זמין. מתחבר מחדש עם הגדרות רגילות."
                    )
                    continue
                ...
                if is_net_err:
                    ...
                    self.ui.write_log(
                        f"SYS: החיבור נכשל. ניסיון נוסף בעוד {_conn_backoff} שניות. "
                        "יש לבדוק את חיבור הרשת."
                    )
                else:
                    self._conn_backoff = 3
            finally:
                self.session = None
                # ASR reconnects do not end or summarise the REST conversation.
...
def _cleanup_integrations():
    from core.browser_runtime import shutdown as shutdown_browser
    from core.mcp_client import shutdown_mcp
    from core.skills import shutdown_scripts
    for cleanup in (shutdown_browser, shutdown_scripts, shutdown_mcp):
        try:
            cleanup()
        except Exception as error:
            print(f"[Shutdown] {cleanup.__name__}: {type(error).__name__}")
