...
#
# This list is a preference, not a promise. Output is measured during discovery.
# Input discovery uses metadata; the capture owner validates the selected input
# with _usable(probe_input=True) only after the user enables the microphone.
# On the measured Windows platform that lands on
...
def _transport_works(idx: int, kind: str, api_key, *, probe_input=False) -> bool:
    """Does this host API actually move audio, or only pretend to?

    Probed once per API per direction and cached. Output writes silence. Input
    would activate real capture and therefore requires explicit probe_input.
    Denied input probes return False without caching an availability verdict.
    """
    # Discovery is not consent to activate the microphone. Only the capture
    # owner, after the physical ear button was enabled, may opt into probing.
    if kind == "input" and not probe_input:
        return False
...
def _usable(idx: int, kind: str, *, probe_input=False) -> bool:
    """Can this device actually be opened at the rate we need?

    Input requires explicit probe_input from the capture owner. Once allowed,
...
def _query() -> dict[str, list[str]]:
    ...
    Input availability comes from host metadata only. Opening settings or
    warming this cache must never activate microphone capture. Input transport
    validation is deferred to the capture owner after the user enables ears.
    """
    ...
    try:
            ...
            for idx, dev in enumerate(devices):
                ...
                if api_filter is not None:
                    ...
                    if api_filter not in api:
                        continue
                if kind != "input" and not _usable(idx, kind):
                    continue
                seen.add(name)
        ...
        for kind in ("input", "output"):
            for api_filter in list(preferred) + [None]:
                ...
                if kind != "input" and not _transport_works(found[0][0], kind, (api_filter, kind)):
                    continue
...
def prefetch() -> None:
    ...
    Microphone discovery is passive; it never opens an input stream.
    """
    def _work():
        global _cache
        result = _query()
...
def resolve(name: str, kind: str, *, probe_input=False):
    ...
    Input lookup is passive unless the capture owner explicitly sets
    probe_input=True after the user enables the microphone.
    ...
    try:
        ...
        for api_filter in orders:
            ...
            for idx, dev_name in _candidates(api_filter):
                if dev_name == wanted:
                    if (kind == "input" and not probe_input) or _usable(idx, kind, probe_input=probe_input):
                        return idx
                    continue
