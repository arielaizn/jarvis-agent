...
### Codex CLI בתוך ג׳רוויס

בהגדרות אפשר לבחור **Codex** או **Gemini** בלחיצה. הבחירה נשמרת גם להפעלה הבאה. Codex מפעיל את `gpt-6-astra` עם `model_reasoning_effort="high"` דרך ההתחברות המקומית ל-ChatGPT. אין החלפה אוטומטית למודל אחר.

במצב Codex, פקודה קולית או כתובה מפעילה משימה ברקע. ההתקדמות מוצגת במסך ואפשר לעצור את המשימה. עצירה אינה מבטלת פעולות שכבר בוצעו. בקשות שעוסקות בהערות עוברות במסלול המקורות של הגלקסיה; אפשר לבחור במסך "שאלה על ההערות". פקודות מיקוד, שמירת הערה ושיתוף מסך ממשיכות לפעול דרך הבקר המקומי.

המשימות מקבלות את הגדרות ה-MCP והסקילים של Codex המותקן. הרצת פקודות מוגבלת ל-`workspace-write`, ותיקיית העבודה היא תיקיית ג׳רוויס. כשהרשאת מערכת או מגבלת סביבת העבודה חוסמת פעולה, Codex צריך לדווח על החסימה. פעולה שמחייבת אישור אינטראקטיבי ב-CLI עשויה לדרוש המשך בחלון Codex.

המרת קול לטקסט עדיין משתמשת ב-Gemini Live ובמפתח ששמור בהגדרות. החשיבה, משימות Codex ותשובות על תמונות משתמשות במוח שנבחר. המיקרופון נפתח רק מכפתור האוזן.

בדיקה חיה: `.venv/bin/python preflight.py`. במצב Codex הבדיקה מאמתת גם יצירת קובץ אמיתי ב-`.artifacts/codex-verification/` באמצעות משימת CLI. בדיקות תמונה והערות מפעילות את CLI במצב קריאה בלבד, בלי לטעון את שרתי ה-MCP של משימות הביצוע.

הקול המקומי מוגדר לפרופיל "גברי עמוק" בעברית, עם גובה צליל נמוך וקצב רגוע. אפשר לשנות אותו דרך התאמה אישית ולחזור ל"הקול המקורי". הפרופילים מבוססים על קול המערכת בעברית, וההקראה פועלת על המחשב. הטקסט מועבר דרך stdin ואינו נשמר בקובץ הקראה.
